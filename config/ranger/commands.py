...
# fzf_locate: The following snippet searches your home directory for files and directories with fzf.
...
class fzf_rga_documents_search(Command):
    ...
    Usage: fzf_rga_documents_search <search string>
    """
    def execute(self):
        if self.arg(1):
            search_string = self.rest(1)
        else:
            self.fm.notify("Usage: fzf_rga_documents_search <search string>", bad=True)
            return
...
class YankContentWl(Command):
    # Works with GIMP but not web browsers
    def get_file(self):
    ...
    def execute(self):
        ...
        if file.image or file.mimetype.startswith('text'):
            with open(file.path, 'r') as f:
                command = ['wl-copy']
                if file.mimetype:
                    command.extend(['-t', file.mimetype])
                subprocess.run(command, stdin=f, check=True)
        else:
            self.fm.notify("{} is not an image file or a text file".format(file.relative_path))

class YankUriWl(YankContentWl):
    # Works with web browsers but not with GIMP
    def execute(self):
        try:
            file = self.get_file()
        except RuntimeError:
        ...
        uri = "file://" + urllib.parse.quote(file.path)
        subprocess.run(["wl-copy", "-t", "text/uri-list", uri], check=True)


# :cdf [path_to_dir]
# Counts all files in the path_to_dir directory (and all subdirectories).
class cdf(Command):
    def execute(self):
        import os
        if not self.arg(1):                                                
            dir_ = str(self.fm.thisdir)
        else:
            dir_ = str(self.arg(1))
        files_num = 0
        for dir_path, dir_names, file_names in os.walk(dir_):
            files_num += len(file_names)
        self.fm.notify(f'\n\r{files_num} files are in {dir_}')
